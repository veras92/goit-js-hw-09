...
const options = {
  ...
  onClose(selectedDates) {
    ...
    const timmeDifference = selectedDates[0].getTime() - Date.now();
    if (timmeDifference <= 0) {
...
function onStartBtnClick() {
  intervalId = setInterval(() => {
    const timmeDifference = datePickr.selectedDates[0].getTime() - Date.now();
    if (timmeDifference <= 1000) {
      clearInterval(intervalId);
      Notify.success('Time is out!');
    }
    const countingTime = convertMs(timmeDifference);
